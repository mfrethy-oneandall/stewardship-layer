...
A stewardship layer sits between an agent and the systems it can change. It enforces the loop: **Propose → Explain → Confirm → Execute → Learn**. No action runs without explicit steward consent (human or authorized agent) unless it falls inside a pre-declared safe domain with guardrails.
...
- A steward (human or authorized agent/policy) must confirm or deny. Default: deny.
...
3. **Confirm**: Require an explicit APPROVE/DENY; record which steward decided and why. Allowlists may auto-approve only within safe domains.
...
- Governed-by-stewards by default; zero autonomous mutations outside safe domains.
